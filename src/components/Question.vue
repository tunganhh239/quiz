...
<script>
  export default{
      ...
      methods: {
        generateQuestion(){
          const firstNumber=this.generateNumber(1,100);
          const secondNumber=this.generateNumber(1,100);

          let correctAnswer=0;

          switch (this.generateNumber(1,2)) {
              case 1:
            ...
            this.btnData[0].answer = this.generateNumber(correctAnswer - 10, correctAnswer + 10, correctAnswer);
            this.btnData[0].correct = false;
            this.btnData[1].answer = this.generateNumber(correctAnswer - 10, correctAnswer + 10, correctAnswer);
            this.btnData[1].correct = false;
            this.btnData[2].answer = this.generateNumber(correctAnswer - 10, correctAnswer + 10, correctAnswer);
            this.btnData[2].correct = false;
            this.btnData[3].answer = this.generateNumber(correctAnswer - 10, correctAnswer + 10, correctAnswer);
            this.btnData[3].correct = false;

            const correctButton= this.generateNumber(0,3);
            this.btnData[correctButton].correct=true;
            this.btnData[correctButton].answer=correctAnswer;

        },
        generateNumber(min, max, expect){
          const randomNumber= Math.round(Math.random()*(max-min))+min;
          console.log(randomNumber);
          if(randomNumber==expect){
            return this.generateNumber(min,max,expect);
          }
          return randomNumber;
        },
        onAnswer(isCorrect) {
